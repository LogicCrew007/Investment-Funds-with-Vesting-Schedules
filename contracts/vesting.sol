/**
 *Submitted for verification at testnet.bscscan.com on 2023-09-06
 */
...
contract Vesting {
    ...
    function getClaimable(
        ...
        if (percentageOnStart != 0) {
            uint256 claimableOnStart = (percentageOnStart * amount) /
                MAX_PERCENTAGE;
            uint256 claimableRest = (elapsed * (amount - claimableOnStart)) /
                (end - start);
...
contract VestingFactory is Ownable, ReentrancyGuard, AccessControl {
    ...
    function handleFees(address token) internal returns (uint256) {
        ...
        if (msg.value < feeInEth) {
            ...
            if (((feeInEth - msg.value) * 100) / feeInEth > 5)
                revert FeeNotMet(feeInEth, msg.value);
    ...
    function updateWhitelist(address wallet, bool noFee) external nonReentrant {
        if (!hasRole(WHITELIST_ROLE, msg.sender) && owner() != msg.sender) {
